...
def on_bar(context, bars):
    ...
    elif position:TA_Lib-0.4.19-cp39-cp39-win_amd64.whl
        if MA_short[-1] < MA_long[-1] and MA_short[-2] > MA_long[-2]:
...
if __name__ == '__main__':
    ...
    print(info)
    info = pd.DataFrame(np.array(info), columns=['pnl_ratio', 'pnl_ratio_annual', 'sharp_ratio', 'max_drawdown', 'short', 'long'])
